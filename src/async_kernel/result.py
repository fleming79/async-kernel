...
__all__ = ["InvalidStateError", "Result", "ResultCancelledError"]
...
class ResultCancelledError(anyio.ClosedResourceError):
    "Used to indicate a result is cancelled."


class InvalidStateError(RuntimeError):
    "An invalid state of a result."


class Result(Awaitable[T]):
    """
    A result is thread-safe hybrid synchronous and asynchronous cancellable Future.
    """
    ...
    "Keys of metadata to omit when creating a repr of the result."

    _metadata_mappings: ClassVar[dict[int, dict[str, Any]]] = {}
    "A mapping of result's id its metadata."
    ...
    def metadata(self) -> dict[str, Any]:
        """
        The metadata passed as keyword arguments to the result during creation.
        """
    ...
    def __repr__(self) -> str:
        rep = "<Result" + (" ⛔" if self.cancelled() else "") + (" 🏁" if self._done else " 🏃")
    ...
    async def wait(self, *, timeout: float | None = None, shield: bool = False, result: bool = True) -> T | None:
        """
        Wait for result to be done (thread-safe) returning the result if specified.

        Args:
            timeout: Timeout in seconds.
            shield: Shield the result from cancellation.
            result: Whether the result should be returned (use `result=False` to avoid exceptions raised by [Result.result][]).
        """
        try:
    ...
    def cancel(self, msg: str | None = None) -> bool:
        ...
        Notes:
            ...
            - The result will not be *done* until either [Result.set_result][] or [Result.set_exception][] is called.
    ...
    def done(self) -> bool:
        """
        Returns True if the result has a result.
    ...
    def result(self) -> T:
        ...
        Raises:
            ResultCancelledError: If the result has been cancelled.
    ...
    def exception(self) -> BaseException | None:
        """
        Return the exception that was set on the result.
        ...
        if hasattr(self, "_cancelled"):
            raise ResultCancelledError(self._cancelled)
        if not self._done:
            raise InvalidStateError
