...
class Poll:
    ...
    _ctrl_sock: None | zmq.Socket = None
    ...
    def execute(self, func: Callable[[], T]) -> Pending[T]:
        """
        Execute func in this objects `thread`.

        Notes:
            - Use this for thread-safe operations from any thread (including this object's).
            - It is recommended to await / wait_sync / wait the result when the call is made.
        """
        if self.stopped.done():
            msg = "Stopped"
            raise RuntimeError(msg)

        def do_execute() -> Pending[T]:
            if not pen.done():
                try:
                    pen.set_result(func())
                except BaseException as e:
                    pen.set_exception(exception=e)
            return pen

        pen = Pending[T]()
        if threading.current_thread() is self.thread:
            return do_execute()
        self._execute.append(do_execute)
        self._wakeup_thread()
        return pen
