...
__all__ = ["InvalidStateError", "Pending", "PendingCancelled", "PendingContext", "PendingManager"]
...
class Pending(Awaitable[T]):
    ...
    async def wait(self, *, timeout: float | None = None, protect: bool = False, result: bool = True) -> T | None:
        ...
        try:
            ...
            if not protect:
                self.cancel(f"Cancelled due to cancellation error {e}.")
            raise
...
class PendingTracker:
    ...
    def current(cls) -> Self | None:
        ...
        if (id_ := cls._contextvar.get()) and (current := cls._active_contexts.get(id_)):
            if current._state is PendingTrackerState.exiting:
                msg = f"The current {cls.__name__} is exiting!"
                raise InvalidStateError(msg)
            return current
        return None

    def add_pending(self, pen: Pending) -> None:
        "Add a `Pending` to the managers 'pending' set."
        if not pen.done():
            if self._state not in {PendingTrackerState.idle, PendingTrackerState.active}:
                msg = "{self.__class__.__name__} is not active!"
                raise InvalidStateError(msg)
            if pen not in self._pending:
                pen.add_done_callback(self.remove_pending)
                self._pending.add(pen)
                self._count_event.up()

    def remove_pending(self, pen: Pending) -> None:
        "Remove a `Pending` from the 'pending' set."
        try:
            self._pending.remove(pen)
            self._count_event.down()
            if (
                self._count_event.value
                and (self._state is PendingTrackerState.active)
                and (pen.cancelled() or pen.exception())
            ):
                for pen in self._pending.copy():
                    pen.cancel(f"The async context of {self.__class__} has been cancelled")
            if self._state is PendingTrackerState.exiting and not self._count_event.value:
                self._state = PendingTrackerState.stopped
        except KeyError:
            pass

    def discard_pending(self, pen: Pending) -> None:
        "Discard a `Pending` from the 'pending' set."
        if pen.remove_done_callback(self.remove_pending):
            self.remove_pending(pen)
...
class PendingManager(PendingTracker):
    ...
    def deactivate(self, *, cancel_pending: bool = False) -> CountdownEvent | None:
        """
        ...
        self._active_contexts.pop(self.context_id)
        for pen in self._pending.copy():
            if cancel_pending:
                pen.cancel(f"The async context of {self.__class__} has been cancelled")
            else:
                self.discard_pending(pen)
...
class PendingContext(PendingTracker, anyio.AsyncContextManagerMixin):
    """
    PendingContext is an asynchronous context manager that works roughly like a taskgroup.

    Features:
        - New pending created within the context (including sub-threads) are tracked.
        - The context will exit after all tracked pending are done.
        - Cancelled or failed pending initiate cancellation of unfinished pending.

    Usage:
        Enter the async context and create new pending, such as  with  [Caller][async_kernel.caller.Caller].

        ```python
        async with PendingContext() as pc:
            Caller().to_thread(...)
            Caller().call_soon(...)
            Caller().queue_call(...)
        ```
    ...
    Raises:
        InvalidStateError: If the context is entered in an invalid state.
        NotImplementedError: For disabled methods.
    """
