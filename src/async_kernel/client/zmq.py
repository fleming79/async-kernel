...
class ZMQKernelClient(BaseKernelClient, ConnectionFileMixin):  # pyright: ignore[reportUnsafeMultipleInheritance]
    ...
    async def _heartbeat(self) -> None:
        ...
        with self.open_socket(Channel.heartbeat) as sock, self.zmq_poll.event_handler(sock, recv):
            while True:
                count = count + 1
                sock.send(b"ping")
                await async_sleep(1)
    ...
    async def _wait_for_ready(self) -> None:
        with self.open_socket(Channel.iopub) as iopub:
            ...
            with self.zmq_poll.event_handler(iopub, lambda _, __: None, count=(1, resume.wake)):
                await resume
            iopub.unsubscribe(b"")
    ...
    async def iopub_subscribe(self, topic=b"") -> AsyncGenerator[SingleAsyncQueue[Message]]:
        ...
        Tip:
            - A sync version of this function can be achieved by using zmq_poll directly.
        """
        ...
        with (
            ...
            self.zmq_poll.event_handler(iopub, forward_messages, canceller=canceller),
        ):
