...
async def test_kernel_get_handler_invalid(kernel: Kernel):
    with pytest.raises(TypeError, match="A handler was not found for msg_type="):
        kernel.get_handler("invalid mode")  # pyright: ignore[reportArgumentType]


@pytest.mark.parametrize("msgtype", MsgType)
async def test_kernel_get_handler(kernel: Kernel, msgtype: MsgType):
    handler = kernel.get_handler(msgtype)
