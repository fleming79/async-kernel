...
class Test_zmq_Poll:
    ...
    async def test_zmq_poll(self, anyio_backend: Backend, caller: Caller):
        ...
        with Poll() as poll:
            ...
            await poll.execute(lambda: sock_dealer.connect(addr))
    ...
    async def test_poll_limit(self, caller: Caller):
        ...
        with Poll() as poll:
            ...
            await poll.execute(lambda: sock_router.bind(addr))
            await poll.execute(lambda: sock_dealer.connect(addr))
    ...
    def test_execute(self):
        def f(func):
            return poll.execute(func).wait_sync(timeout=tests.utils.TIMEOUT)

        with Poll() as poll:
            assert poll.execute(lambda: 1 + 1).wait_sync() == 2
            assert poll.execute(lambda: f(lambda: 2 + 2)).wait_sync(timeout=tests.utils.TIMEOUT) == 4

        with pytest.raises(RuntimeError):
            poll.execute(lambda: None)
