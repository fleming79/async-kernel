...
@pytest.mark.parametrize(
    ("kernel_name", "interface_factory"),
    [
        (KernelName.trio, "async_kernel.kernel.Kernel"),
        ("function_factory", "function"),
    ],
)
def test_write_kernel_spec(kernel_name: KernelName, interface_factory, tmp_path):
    if interface_factory == "function":

        def my_interface_factory(settings):
            from async_kernel.kernel import Kernel  # noqa: PLC0415

            class MyKernel(Kernel):
                pass

            return MyKernel(settings).interface

        interface_factory = my_interface_factory

    path = write_kernel_spec(tmp_path, kernel_name=kernel_name, interface_factory=interface_factory)
...
def test_write_kernel_spec_fails():
    with pytest.raises(ValueError, match="not enough values to unpack"):
        write_kernel_spec(kernel_name="never-works", interface_factory="not a factory")
